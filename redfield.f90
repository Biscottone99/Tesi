program red2
    ...
    integer a,b,c,d,e,i,j,k,ii,jj,kk,iii,jjj,kkk,dim,conta,info,&
            lwork,nt,dimsq,ab,cd,nkrylov,it, nsiti, isiti
    double precision pi,temp,hbar,kb,kbt,norm,gamma,dt,time, eta, cutoff
    double precision, allocatable :: w(:),work(:),spectral(:,:),bedistr(:,:), sdr(:,:)
    doublecomplex imag,trace,overlap,entime, muxtime, muytime, muztime
    doublecomplex, allocatable :: psi0(:),denmat(:,:),redfield(:,:,:,:),denvec(:),&
            lmat(:,:), mux(:,:), muy(:,:), muz(:,:), sdrtime(:)
    ...
    allocate(w(dim), mux(dim,dim), muy(dim,dim), muz(dim,dim),sdr(dim,nsiti),psi0(dim))
    ...
    do i = 1,dim
        do j = 1,dim
            if (dabs(w(i)-w(j)).gt.1.d-9) then
                ...
                ! if (w(i).gt.w(j)) spectral(i,j) = gamma
                if(w(j).gt.w(i)) spectral(i,j) = (hbar**2/pi)*(((w(j)-w(i))/cutoff)/( 1+ (w(j)-w(i))**2/cutoff**2))*eta
                ! if (w(j).gt.w(i)) spectral(i,j) = -gamma
                if(w(j).gt.w(i)) spectral(i,j)=-(hbar**2/pi)*(((w(j)-w(i))/cutoff)/( 1+ (w(j)-w(i))**2/cutoff**2))*eta
            end if
        end do
    end do
            ...
            do c = 1,dim
                do d = 1,dim
                    ...
                    if (b==d) then
                        do e = 1,dim
                            redfield(a,b,c,d) = redfield(a,b,c,d) - &
                                    gammap(mux,mux,spectral,bedistr,w,a,e,e,c,dim)
                            redfield(a,b,c,d) = redfield(a,b,c,d) - &
                                    gammap(muy,muy,spectral,bedistr,w,a,e,e,c,dim)
                            redfield(a,b,c,d) = redfield(a,b,c,d) - &
                                    gammap(muz,muz,spectral,bedistr,w,a,e,e,c,dim)
                        end do
                    end if
                    ...
                    if (a==c) then
                        do e = 1,dim
                            redfield(a,b,c,d) = redfield(a,b,c,d) - &
                                    gammam(mux,mux,spectral,bedistr,w,d,e,e,b,dim)
                            redfield(a,b,c,d) = redfield(a,b,c,d) - &
                                    gammam(muy,muy,spectral,bedistr,w,d,e,e,b,dim)
                            redfield(a,b,c,d) = redfield(a,b,c,d) - &
                                    gammam(muz,muz,spectral,bedistr,w,d,e,e,b,dim)
                        end do
                    end if

                    redfield(a,b,c,d) = redfield(a,b,c,d) + &
                            gammap(mux,mux,spectral,bedistr,w,d,b,a,c,dim)
                    redfield(a,b,c,d) = redfield(a,b,c,d) + &
                            gammam(mux,mux,spectral,bedistr,w,d,b,a,c,dim)
                    redfield(a,b,c,d) = redfield(a,b,c,d) + &
                            gammap(muy,muy,spectral,bedistr,w,d,b,a,c,dim)
                    redfield(a,b,c,d) = redfield(a,b,c,d) + &
                            gammam(muy,muy,spectral,bedistr,w,d,b,a,c,dim)
                    redfield(a,b,c,d) = redfield(a,b,c,d) + &
                            gammap(muz,muz,spectral,bedistr,w,d,b,a,c,dim)
                    redfield(a,b,c,d) = redfield(a,b,c,d) + &
                            gammam(muz,muz,spectral,bedistr,w,d,b,a,c,dim)
                end do
            end do
        end do
    end do
...
!    end do
    ...
    do it = 1,nt
        ...
        !$omp shared(dim,denvec,mux, muy, muz,w,sdr, nsiti), &
        !$omp reduction(+:muxtime, muytime, muztime, sdrtime, entime)
        do a = 1,dim
