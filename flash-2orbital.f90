...
use module
  ...
  complex*16,allocatable:: bi_coupx(:,:),bi_coupy(:,:), bi_coupz(:,:), temporary(:,:,:,:,:), mcrotx(:,:), bcrotx(:,:),mcroty(:,:), mcrotz(:,:), bcroty(:,:), bcrotz(:,:), soor(:,:), ssor(:,:), cicciobello(:,:,:,:), soc_monox(:,:), soc_monoy(:,:), soc_monoz(:,:),psi0(:),mux(:,:), muy(:,:), muz(:,:),  muarx(:,:), muary(:,:), muarz(:,:), mubrx(:,:), mubry(:,:), mubrz(:,:), mutrrot(:,:) , mutrtb(:,:), mutrconf(:,:)
  ...
  real*8::strenght,hbar,hbarev,echarge,emass,dpg, bubu, balu, delta, gamma, gamma2, tollerance, bibi,norm, mud
...
!!$  enddo
  phase2(1,1)=-1.d0
  phase2(4,4)=phase2(1,1)
  phase2(2,2)=-1d0
  phase2(3,3)=phase2(2,2)
  phase2(2,3)=1d0
  phase2(3,2)=1d0
...
!!$     a=0
!!$     b=0
!!$     if(btest(vecconfig(n),0)) a=a+1
!!$     if(btest(vecconfig(n),1)) a=a+2
!!$     if(btest(vecconfig(n),2)) b=b+1
!!$     if(btest(vecconfig(n),3)) b=b+2
!!$     if(a==0.or.a==3) go to 100
!!$     if(b==0.or.b==3) go to 100
!!$     !if((a/=0.or.a/=3).and.(b/=0.or.b/=3)) write(77,*) a,b,n
!!$     if((a.eq.1).and.(b.eq.1))ham(n,n)=ham(n,n)+exchange(1,1)*phase2(1,1)
!!$     !ham(n,n)=exchange(1,1)*phase2(1,1)
!!$     if((a.eq.2).and.(b.eq.2)) ham(n,n)=ham(n,n)+exchange(4,4)*phase2(4,4)
!!$     if((a.eq.1).and.(b.eq.2)) ham(n,n)=ham(n,n)+exchange(2,2)*phase2(2,2)
!!$     if((a.eq.2).and.(b.eq.1)) ham(n,n)=ham(n,n)+exchange(3,3)*phase2(3,3)!ham(n,n)=exchange(3,3)*phase2(3,3)
!!$    
!!$     if((a.eq.1).and.(b.eq.2))then
!!$        temp=ibclr(vecconfig(n),0)
!!$        temp=ibclr(temp,3)
!!$        temp=ibset(temp,1)
!!$        temp=ibset(temp,2)
!!$        m=binary_search(vecconfig, temp, 1,dim2)
!!$        ham(n,m)=ham(n,m)+exchange(2,3)*phase2(2,3)
!!$     endif
!!$
!!$     if((a.eq.2).and.(b.eq.1))then
!!$        temp=ibclr(vecconfig(n),1)
!!$        temp=ibclr(temp,2)
!!$        temp=ibset(temp,0)
!!$        temp=ibset(temp,3)    
!!$        m=binary_search(vecconfig, temp, 1,dim2)
!!$        ham(n,m)=ham(n,m)+exchange(3,2)*phase2(3,2)
!!$     endif
!!$
!!$     100 continue
!!$  enddo
...
!=========================SCRITTURA HAM=========================
  ...
  do i = 1,nsiti+1
     ...
     do j = 1,nsiti+1
        ...
        do k = 1,3
           bubu = nuclei(sitoi,k)-nuclei(sitoj,k)
           mom(sitoi,sitoj,k) = cplx * bubu * hop(i,j)
        end do
     end do
  end do
...
!!$
  ...
  do n=1,dim2
     ...
     do m=1,dim2
        ...
        ham(n,m)=ham(n,m)-tt(n,m)+exc(n,m)+coup(n,m)*kappa!(coup(n,m)-soo(n,m)-sso(n,m))*kappa
     enddo
  enddo
